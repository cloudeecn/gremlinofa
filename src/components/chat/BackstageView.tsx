...
export interface BackstageViewProps {
  ...
  /** True if this backstage content was generated by a tool (minion, etc.) - uses pinker styling */
...
export default function BackstageView({
  ...
  // Use pinker styling for tool-generated content
  const borderClass = isToolGenerated ? 'border-pink-300' : 'border-purple-400';
  const bgClass = isToolGenerated ? 'bg-pink-50' : 'bg-purple-50';
  const textClass = isToolGenerated ? 'text-pink-800' : 'text-purple-800';
  const hoverClass = isToolGenerated ? 'hover:bg-pink-100' : 'hover:bg-purple-100';
  const accentClass = isToolGenerated ? 'text-pink-600' : 'text-purple-600';
  ...
  const borderSeparatorClass = isToolGenerated ? 'border-pink-200' : 'border-purple-200';

  return (
    <div
      className={`backstage-container overflow-hidden rounded-r-lg border-l-4 ${borderClass} ${bgClass}`}
    >
      {/* Collapsible header */}
      <button
            ...
            {instantBlock ? (
              ...
              <>
                <span className="flex min-w-0 overflow-hidden">
                  <span
                    className={`flex max-w-full justify-end overflow-hidden text-xs font-normal whitespace-nowrap ${accentClass}`}
                  >
                    {getPreviewText()}
                  </span>
                </span>
              ...
              <span
                className={`min-w-0 flex-1 truncate text-xs font-normal whitespace-nowrap ${accentClass}`}
              >
                {getPreviewText()}
              </span>
            )}
      ...
      {/* Expanded content */}
      {isExpanded && (
        <div className={`border-t ${borderSeparatorClass} bg-white px-4 py-3`}>
