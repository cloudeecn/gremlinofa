...
import type { Message, APIDefinition } from '../../../../shared/protocol/types';

// Mock the gremlinClient surface CacheWarning consumes. The component reads
// the API definition + the cached models list and finds the active model
// inside the array; the tests stub both methods.
vi.mock('../../../client', () => ({
  gremlinClient: {
...
describe('CacheWarning', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Default mock: cached model list returns the test model so the
    // component's `find(m => m.id === currentModelId)` lookup hits.
    vi.mocked(gremlinClient.getModelsCache).mockResolvedValue({
      models: [
        {
  ...
  it('should not render when there are no messages', async () => {
    vi.mocked(gremlinClient.getAPIDefinition).mockResolvedValue(mockAnthropicApiDef);
  ...
  it('should not render when not using Anthropic API', async () => {
    vi.mocked(gremlinClient.getAPIDefinition).mockResolvedValue(mockOpenAIApiDef);
