...
import type { ToolInventoryEntry } from '../../../shared/protocol/protocol';
...
export default function ProjectSettingsView({ projectId, onMenuPress }: ProjectSettingsViewProps) {
  ...
  // Tool inventory comes from the backend (the in-process registry lives
  // server-side after the boundary cleanup). One-shot fetch on mount.
  const [availableTools, setAvailableTools] = useState<ToolInventoryEntry[]>([]);
  useEffect(() => {
    let cancelled = false;
    gremlinClient.listTools().then(tools => {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-gray-50">
      ...
      <div className="ios-scroll flex-1 overflow-y-auto overscroll-y-contain p-4">
        ...
        {hasModelSelected && (
          <>
            ...
            <div className="mb-6">
              ...
              <p className="mb-2 text-xs text-gray-500">Ignored by most reasoning models.</p>
              <input
                type="number"
                step="0.1"
                min="0"
        ...
        {/* Remote VFS Section — hidden when connected to a server backend */}
        {getStorageConfig().type !== 'server' && (
