...
// Phase 1.8 leak fix — see VfsFileEditor.tsx for the rationale.
...
export default function VfsManagerView({ projectId, initialPath }: VfsManagerViewProps) {
  ...
  const adapter = useMemo(() => gremlinClient.getVfsAdapter(projectId), [projectId]);
  ...
  const handleDeleteDirectory = useCallback(async () => {
    ...
    const dirname = basename(selectedPath);
    const confirmed = await showDestructiveConfirm(
      'Delete Directory',
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    try {
      ...
      const dirName = selectedPath === '/' ? 'vfs-root' : basename(selectedPath) || 'root';
            ...
            <div className="border-t border-gray-200 bg-white p-4">
              <div className="flex flex-col gap-3">
                <div className="text-sm text-gray-600">
                  Selected: <span className="font-medium">{basename(selectedPath) || '/'}</span>
                </div>
            ...
            {!selectedPath ? (
              ...
              <div className="flex flex-1 flex-col">
                ...
                <div className="flex items-center gap-3 border-b border-gray-200 px-4 py-3">
                  <span className="text-xl">📁</span>
                  <span className="font-medium text-gray-900">{basename(selectedPath) || '/'}</span>
                </div>
                {/* Directory actions */}
