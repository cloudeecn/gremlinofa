...
import type { AttachmentSection, MessageAttachment } from '../../../shared/protocol/types';

// Mock `../client` so the hook calls a stub `gremlinClient` we control
// directly. Replaces the previous pattern of mocking `services/storage`
// and relying on the in-process backend to dispatch through it.
vi.mock('../../client', () => ({
  gremlinClient: {
    getAllAttachmentSections: vi.fn(),
    getAttachments: vi.fn(),
    deleteAttachment: vi.fn(),
    updateMessageAttachmentIds: vi.fn(),
    deleteAttachmentsOlderThan: vi.fn(),
  },
}));
...
describe('useAttachmentManager', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(gremlinClient.getAllAttachmentSections).mockResolvedValue(mockSections);
    vi.mocked(gremlinClient.getAttachments).mockResolvedValue(mockAttachments);
    vi.mocked(gremlinClient.deleteAttachment).mockResolvedValue('msg_1');
    vi.mocked(gremlinClient.updateMessageAttachmentIds).mockResolvedValue(undefined);
    vi.mocked(gremlinClient.deleteAttachmentsOlderThan).mockResolvedValue({
      deleted: 0,
      updatedMessageIds: [],
    });
  });
  ...
  describe('loadSections', () => {
    it('should load sections from gremlinClient', async () => {
      const { result } = renderHook(() => useAttachmentManager());
      ...
      expect(gremlinClient.getAllAttachmentSections).toHaveBeenCalled();
    ...
    it('should handle errors', async () => {
      ...
      vi.mocked(gremlinClient.getAllAttachmentSections).mockRejectedValue(
  ...
  describe('loadSectionData', () => {
    it('should load attachment data for a section', async () => {
      ...
      expect(gremlinClient.getAttachments).toHaveBeenCalledWith('msg_1');
    ...
    it('should skip if already loaded', async () => {
      ...
      expect(gremlinClient.getAttachments).toHaveBeenCalledTimes(1);
      ...
      // Should not call gremlinClient again
      expect(gremlinClient.getAttachments).toHaveBeenCalledTimes(1);
    });
  ...
  describe('deleteSelected', () => {
    it('should delete selected attachments', async () => {
      ...
      expect(gremlinClient.deleteAttachment).toHaveBeenCalledWith('att_1');
      expect(gremlinClient.deleteAttachment).toHaveBeenCalledWith('att_2');
    ...
    it('should update message attachmentIds after deletion', async () => {
      ...
      expect(gremlinClient.updateMessageAttachmentIds).toHaveBeenCalledWith(
        'chat_1',
        'msg_1',
    ...
    it('should handle deletion errors', async () => {
      ...
      vi.mocked(gremlinClient.deleteAttachment).mockRejectedValue(new Error('Delete failed'));
    ...
    it('should reload sections after deletion', async () => {
      ...
      vi.mocked(gremlinClient.getAllAttachmentSections).mockClear();
  ...
  describe('deleteOlderThan', () => {
    it('should call gremlinClient.deleteAttachmentsOlderThan with correct days', async () => {
      const { result } = renderHook(() => useAttachmentManager());
      ...
      expect(gremlinClient.deleteAttachmentsOlderThan).toHaveBeenCalledWith(7);
    ...
    it('should update message attachmentIds for affected messages', async () => {
      ...
      expect(gremlinClient.updateMessageAttachmentIds).toHaveBeenCalled();
    });
