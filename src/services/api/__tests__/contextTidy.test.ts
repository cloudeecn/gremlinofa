...
describe('applyContextTidy', () => {
  ...
  it('skips messages with null fullContent', () => {
    ...
    const result = applyContextTidy(messages, 'cp', new Set<string>(), 'anthropic', testFilter);
  ...
  it('does not mutate original messages', () => {
    ...
    applyContextTidy(messages, 'cp', new Set<string>(), 'anthropic', testFilter);
  ...
  it('preserves checkpoint tool_use when checkpointMessageId points to tool_use assistant message', () => {
    ...
    // a1: pre-checkpoint assistant with filesystem tool (should be tidied)
    ...
    // a1: thinking removed, filesystem tool_use tidied → message dropped (empty)
    // u1: filesystem tool_result tidied → message dropped (empty)
  ...
  it('handles empty tidyToolNames (only thinking removed)', () => {
    const messages = [
      msg('a1', 'assistant', 'anthropic', [
        { type: 'thinking', thinking: 'hmm' },
        { type: 'tool_use', id: 't1', name: 'filesystem', input: {} },
