...
// Mock EncryptionCore — `dataImport.ts` constructs a temp core from the
// supplied source CEK. We replace the constructor with a hand-rolled
// stub so the test can short-circuit decryption to a passthrough.
vi.mock('../../services/encryption/encryptionCore');

async function collectCSV(adapter: StorageAdapter, enc?: EncryptionCore): Promise<string> {
  const lines: string[] = [];
...
describe('Data Export/Import Roundtrip', () => {
  ...
  beforeEach(() => {
    ...
    // Mock EncryptionCore constructor (must use function for 'new' keyword)
    (EncryptionCore as any).mockImplementation(function (this: any) {
