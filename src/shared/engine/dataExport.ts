 ...
 * Data Export Runtime
 *
 * Streams all database tables to CSV format via `adapter.exportPaginated()`.
 * Lives in `src/backend/` because it pulls in storage + encryption — frontend
 * code receives progress events through the `gremlinClient.exportData` RPC
 * (driven by `src/backend/exportRunner.ts`) and uses the type-only callbacks
