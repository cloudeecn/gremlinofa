...
/**
 * Async generator that streams CSV rows from a Blob (or File — File extends Blob).
 * Memory-efficient: processes chunks without loading entire file.
 *
 * Accepts a `Blob` so the backend can pass an in-memory bundle (wrapped from
 * a `Uint8Array`) without going through a `File` object — `File` is the main
 * thread's idea, the worker just sees bytes.
 *
 * @param file - Blob/File to parse
