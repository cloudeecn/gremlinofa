...
// Mock dependencies. After the Phase 4 singleton-encapsulation refactor the
// agentic loop reads `extractToolUseBlocks` / `mapStopReason` /
// `shouldPrependPrefill` from the pure `apiHelpers` module instead of the
// `apiService` singleton, and `sendMessageStream` from
// `options.deps.apiService.sendMessageStream`. The test exposes a single
// `apiService` object whose `sendMessageStream` is the mock the loop will
// call (via the deps stub) and whose `extractToolUseBlocks` /
// `mapStopReason` / `shouldPrependPrefill` getters forward to the apiHelpers
// vi.fn mocks — that way the existing `vi.mocked(apiService.X)` test code
// keeps working unchanged.
vi.mock('../../../engine/lib/apiHelpers', () => ({
  extractToolUseBlocks: vi.fn(),
  mapStopReason: vi.fn((_apiType, reason) => reason || 'end_turn'),
...
async function* mockToolGenerator(
  ...
  import('../../../protocol/types').ToolStreamEvent,
  import('../../../protocol/types').ToolResult
> {
  return result as import('../../../protocol/types').ToolResult;
}
...
// Per-instance tool registry stub used by both `createMockDeps()` and any
// helper that previously read from the module-level `toolRegistry` singleton.
// Mirrors the previous behavior: every tool name resolves with the same
// generic shape, except `minion` which is marked complex so the loop's
// simple-vs-complex split test still works.
...
import {
  ...
  mapStopReason as mockMapStopReason,
  shouldPrependPrefill as mockShouldPrependPrefill,
} from '../../../engine/lib/apiHelpers';
import { executeClientSideTool } from '../../tools/clientSideTools';

// `apiService` exposes the same surface the loop touches via `options.deps`.
// `sendMessageStream` is the deps mock the loop calls; the rest are getter
// forwards to the apiHelpers vi.fn mocks so existing test code that does
// `vi.mocked(apiService.extractToolUseBlocks).mockReturnValue(...)` still
// configures the same mock the loop reads from.
const apiService = {
  sendMessageStream: vi.fn(),
  get extractToolUseBlocks() {
    return mockExtractToolUseBlocks;
  },
  get mapStopReason() {
    return mockMapStopReason;
  },
  get shouldPrependPrefill() {
    return mockShouldPrependPrefill;
  },
};
...
const createMockDeps = (): import('../../../engine/backendDeps').BackendDeps => ({
  storage: {
    getAttachments: vi.fn(async () => []),
  } as unknown as import('../../storage/unifiedStorage').UnifiedStorage,
  encryption: {} as unknown as import('../../encryption/encryptionCore').EncryptionCore,
  apiService: apiService as unknown as import('../../api/apiService').APIService,
  toolRegistry:
    toolRegistryStub as unknown as import('../../tools/clientSideTools').ClientSideToolRegistry,
  // Real instance — pure in-memory state. The agentic loop tests don't
  // exercise child-loop registration directly (that's a minion tool
  // concern), but `ToolContext.loopRegistry` is required so the field
  // must point at a usable LoopRegistry.
...
const createMockOptions = (overrides?: Partial<AgenticLoopOptions>): AgenticLoopOptions => ({
  ...
  // on `AgenticLoopOptions`. The agentic-loop tests stub it with a no-op
  // factory because nothing they exercise reaches into the adapter
  // (tools that would call vfsAdapter are mocked).
  createVfsAdapter: () => ({}) as unknown as import('../../vfs/vfsAdapter').VfsAdapter,
  deps: createMockDeps(),
  ...overrides,
});
...
describe('agenticLoopGenerator', () => {
  ...
  describe('runAgenticLoop', () => {
    ...
    describe('hard-abort', () => {
      it('returns aborted status with synthesized incomplete partial when signal fires mid-stream', async () => {
        const controller = new AbortController();

        vi.mocked(apiService.sendMessageStream).mockImplementation(() => {
          return (async function* mockAbortableStream() {
            yield { type: 'content', content: 'partial response' };
            // Simulate the user clicking hard-abort while streaming. The
            // generator's throwIfAborted() between chunks will trigger the
            // abort path before the next yield is processed.
            controller.abort();
            yield { type: 'content', content: 'never reaches the result' };
            return {
              textContent: 'should not be used',
              fullContent: [],
              stopReason: 'end_turn',
              inputTokens: 0,
              outputTokens: 0,
        ...
        expect(assistantMsg?.incomplete).toBe(true);
        expect(assistantMsg?.content.stopReason).toBe('aborted');
      });

      it('aborts immediately at the iteration boundary when signal is already fired', async () => {
        const controller = new AbortController();
        controller.abort();

        // Stream should never even be invoked because the iteration-top
        // throwIfAborted() trips first.
        const sendStreamSpy = vi.mocked(apiService.sendMessageStream);
        ...
        expect(result.status).toBe('aborted');
        // No assistant message synthesized when no stream ever started.
        expect(result.messages.find(m => m.role === 'assistant')).toBeUndefined();
        expect(sendStreamSpy).not.toHaveBeenCalled();
      });

      it('forwards options.signal and options.loopId into ToolContext for tools (e.g. minionTool)', async () => {
        const parentController = new AbortController();
        const PARENT_LOOP_ID = 'loop_parent_123';
        ...
        vi.mocked(apiService.sendMessageStream).mockReturnValue(
          createMockStream([{ type: 'content', content: '' }], {
            textContent: '',
            fullContent: toolUseBlocks,
            stopReason: 'tool_use',
            inputTokens: 1,
        ...
        let capturedContext: import('../../../protocol/types').ToolContext | undefined;
        vi.mocked(executeClientSideTool).mockImplementation((_n, _i, _e, _o, ctx) => {
          capturedContext = ctx;
          return mockToolGenerator({
            content: 'minion done',
            breakLoop: { returnValue: 'ok' },
          });
        });
        ...
        const options = createMockOptions({
          ...
          loopId: PARENT_LOOP_ID,
        });

        await collectAgenticLoop(runAgenticLoop(options, [createMockUserMessage('go')]));

        expect(capturedContext).toBeDefined();
        // The same AbortSignal instance reaches the tool — when the parent
        // hard-aborts, the minion's recursive runAgenticLoop sees it too.
        expect(capturedContext!.signal).toBe(parentController.signal);
        // The parent loop id reaches the tool so minionTool can forward it
        // as parentLoopId on the child loop options.
        expect(capturedContext!.loopId).toBe(PARENT_LOOP_ID);
      });
    });
  });
  ...
  describe('multi-iteration scenarios (stateful mock)', () => {
    ...
    it('recovers tool_use blocks missing from renderingContent using fullContent', async () => {
      ...
      const rendering = firstAssistant.content
        .renderingContent as import('../../../protocol/types/content').RenderingBlockGroup[];
      ...
      expect(
        backstageBlocks.map(
          b => (b as import('../../../protocol/types/content').ToolUseRenderBlock).id
        )
    ...
    it('propagates checkpoint boundary to sendMessageStream on auto-continued iteration', async () => {
      ...
      let idCounter = 0;
      const { generateUniqueId } = await import('../../../protocol/idGenerator');
