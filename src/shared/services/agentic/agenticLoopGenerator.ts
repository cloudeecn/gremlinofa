...
export interface AgenticLoopOptions {
  ...
  // Identity for the loop run. The backend mints `loopId` when it starts a run
  // (PR 7+); minions and other sub-loops carry the parent's loopId in
  // `parentLoopId` so the LoopRegistry can render parent/child relationships.
  // Both are optional today and only used for threading; the loop body still
  ...
  // VFS adapter factory — creates adapters bound to project+userId for any
  // namespace. Always supplied by `buildLoopOptions` from `BackendDeps`.
  // VFS access lives only in the worker; the agentic loop never builds
  // its own adapter.
  createVfsAdapter: import('../../protocol/types').VfsAdapterFactory;
   ...
   * message marked `incomplete: true`, and returns `{ status: 'aborted' }`.
   * Required so the contract is unambiguous; tests pass `new AbortController().signal`.
   */
  signal: AbortSignal;

  /**
   * Injected backend dependencies (storage, encryption, api service, tool
   * registry). The agentic loop reads attachments via `deps.storage` and
   * forwards the bundle into `ToolContext` so tools can pull from injection
   * instead of singleton imports. Required because the loop only runs from
   * `ChatRunner`, which always builds a real bundle. Tests construct a stub.
   */
  deps: BackendDeps;
}
...
export { createTokenTotals, addTokens } from '../../engine/lib/tokenTotals';
export type { TokenTotals } from '../../protocol/types/content';
...
function mergeToolUseInputFromFullContent(
  ...
  const toolUseBlocks = extractToolUseBlocks(apiType, fullContent);
...
/**
 * Populate rendered fields on tool blocks for display persistence. The
 * `registry` parameter is optional so callers without a tool registry
 * (e.g. the abort path's partial-message synthesizer if invoked from a
 * test fixture) get the icon/text fallbacks instead of crashing on a
 * missing registry — same shape `createToolResultRenderBlock` follows.
 */
function populateToolRenderFields(
  groups: RenderingBlockGroup[],
  registry?: ClientSideToolRegistry
): void {
  for (const group of groups) {
    for (const block of group.blocks) {
      if (block.type === 'tool_use') {
...
function buildDummyAssistantMessage(
  text: string,
  toolCalls: { id?: string; name: string; input: Record<string, unknown> }[] | undefined,
  ...
  populateToolRenderFields(renderingContent, registry);
 ...
 * Optionally accepts renderingGroups from tool's internal work. The
 * `registry` parameter is optional so the frontend's `useChat` cancel
 * path can build error blocks without a worker round-trip — when no
 * registry is supplied the icon defaults to ❌/✅ and `renderedContent`
 * passes through unchanged, which is the right shape for an error.
 */
...
function buildAssistantMessage(
  ...
  const { renderingContent, stopReason } = result.error
    ...
    : {
        ...
        stopReason: mapStopReason(apiType, result.stopReason ?? null),
      };
  ...
  const extractedToolCalls = extractToolUseBlocks(apiType, result.fullContent);
...
> {
  ...
  for (const block of executableBlocks) {
    if (toolContext.toolRegistry.get(block.name)?.complex) {
  ...
  const pendingRenderBlocks: ToolResultRenderBlock[] = toolUseBlocks.map(toolUse => ({
    ...
    icon: toolContext.toolRegistry.get(toolUse.name)?.iconOutput ?? '⏳',
  }));
...
export async function* runAgenticLoop(
  ...
  // reaching this code path without one is an upstream bug — fail loudly
  // instead of silently constructing a fallback adapter.
  if (!options.createVfsAdapter) {
    throw new Error(
      'agenticLoopGenerator: AgenticLoopOptions.createVfsAdapter is required. ' +
        'VFS access is only available inside the worker — direct service-layer calls are unsupported.'
    );
  }
  const adapterFactory = options.createVfsAdapter;
  ...
  const toolContext: ToolContext = {
    ...
    storage: options.deps.storage,
    encryption: options.deps.encryption,
    apiService: options.deps.apiService,
    toolRegistry: options.deps.toolRegistry,
    // Tools that spawn child loops (minionTool) read this to register +
    // end the child in the per-server LoopRegistry — that's how parallel
    // minions show up as indented rows under the parent in the sidebar
    // Running Loops section.
    loopRegistry: options.deps.loopRegistry,
  };
  ...
  // In-flight assembler tracker. The catch block reads this to synthesize a
  // partial `incomplete` assistant message when the loop is hard-aborted in
  // the middle of a stream. Set immediately before each stream call, cleared
  // as soon as the assistant message is committed (so we don't double-emit).
  let inFlightAssembler: StreamingContentAssembler | null = null;
  ...
  try {
    ...
    while (iteration < MAX_ITERATIONS) {
      // Hard-abort fast path: bail out before doing any more work for this
      // iteration. The catch handler turns the thrown AbortError into the
      // `aborted` terminal status (with no partial message synthesis here,
      // since no assembler is in flight at this point).
      ...
      if (!hookIntercepted) {
        ...
        // Load attachments for user messages via the injected storage
        // (kills the latent worker-mode singleton bug for the attachment path).
        const messagesWithAttachments = await loadAttachmentsForMessages(
          messages,
          options.deps.storage
        );
        ...
        inFlightAssembler = assembler;

        // Start API stream
        const stream = options.deps.apiService.sendMessageStream(
        ...
        // Process stream chunks. The first await throws synchronously if the
        // signal is already aborted; subsequent iterations re-check between
        // chunks so a hard-abort takes effect at the next chunk boundary even
        // if the underlying SDK swallows the signal.
        options.signal.throwIfAborted();
        ...
        if (result.stopReason !== 'tool_use') {
          ...
          if (options.fallbackToolExtraction) {
            fallbackExtractedBlocks = extractToolUseBlocks(apiDef.apiType, result.fullContent);
    ...
    // Hard-abort path: the controlling AbortController fired. Synthesize a
    // display-only partial assistant message from whatever the in-flight
    // assembler captured (if any), mark it `incomplete: true`, and return
    // the `aborted` terminal status. The chat is then locked from
    // continuation via assertChatNotLockedByIncompleteTail() — the user must
    // delete the incomplete message or roll back to a checkpoint.
    if (options.signal.aborted || isAbortLikeError(error)) {
      if (inFlightAssembler) {
        const partialGroups = inFlightAssembler.finalize();
        populateToolRenderFields(partialGroups, options.deps.toolRegistry);
        const partialText = extractTextFromGroups(partialGroups);
        const partialMsg: Message<unknown> = {
          id: generateUniqueId('msg_assistant'),
          role: 'assistant',
          content: {
            type: 'text',
            ...
            modelFamily: apiDef.apiType,
            fullContent: undefined,
            renderingContent: partialGroups,
            stopReason: 'aborted',
          },
          timestamp: new Date(),
          incomplete: true,
        ...
        yield { type: 'message_created', message: partialMsg };
        inFlightAssembler = null;
      }
      console.debug('[agenticLoopGen] Hard-aborted at iteration', iteration);
      return {
        status: 'aborted',
        messages,
        tokens: totals,
...
/**
 * Detect AbortError-shaped exceptions emitted by the platform fetch / SDKs
 * when an AbortSignal fires. We can't rely on `instanceof DOMException`
 * because some SDKs throw plain Error objects with the right `name`.
 */
...
/**
 * Extract concatenated text from rendering groups for the partial-assistant
 * message synthesized by the abort path. We only walk text-render blocks;
 * thinking blocks are intentionally excluded so the visible message text
 * matches what the user actually saw on screen.
 */
function extractTextFromGroups(groups: RenderingBlockGroup[]): string {
  let text = '';
  for (const group of groups) {
    for (const block of group.blocks) {
