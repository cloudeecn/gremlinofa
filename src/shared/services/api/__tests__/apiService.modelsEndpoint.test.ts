...
// Construct a fresh service per file so this test no longer depends on
// the (now-deleted) module-level `apiService` singleton. The deps are
// stubbed because `discoverModels` against a `modelsEndpoint` goes
// through the global `fetch` mock and never reaches storage / encryption.
const apiService = new APIService(stubApiDeps);
