...
/**
 * Create a mock encryption service. Surface mirrors `EncryptionCore` (the
 * narrower type that backend code consumes) plus the browser-only
 * convenience methods so existing tests that exercise both flavors keep
 * working without churning every spy.
 */
export function createMockEncryptionService() {
  return {
    ...
    forget: vi.fn().mockImplementation(() => undefined),
    hasSameKeyAs: vi.fn().mockReturnValue(false),
    ...
    // Browser-only convenience surface (kept for tests that exercise the
    // legacy `EncryptionService` flavor)
    initialize: vi.fn().mockResolvedValue(undefined),
