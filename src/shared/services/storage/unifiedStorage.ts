...
export class UnifiedStorage {
  ...
  /**
   * `GremlinServer.init()` constructs a fresh `UnifiedStorage` paired with
   * a freshly-keyed `EncryptionCore`. The encryption instance MUST already
   * have a CEK loaded (via `initializeWithCEK`) before `initialize()` is
   * called — `UnifiedStorage` never reaches into localStorage to bring up
   * encryption itself. The Phase 1.5 split makes that contract type-level:
   * the field is typed as `EncryptionCore`, which has no `getCEK()` /
   * `clearCEK()` / `initialize()` methods.
   */
  constructor(adapter: StorageAdapter, encryption: EncryptionCore) {
  ...
  /**
   * Purge all data — delete every database row and forget the in-memory
   * CEK. Caller-side cleanup (clearing the localStorage CEK string,
   * triggering OOBE on the next reload, clearing draft text via
   * `clearAllDrafts`) is the frontend's responsibility: `UnifiedStorage`
   * runs in the worker and has no access to `localStorage`.
   * `DataManagerPage` already calls `clearAllDrafts()` after invoking
   * `gremlinClient.purgeAllData()`. WARNING: irreversible.
   */
  async purgeAllData(): Promise<void> {
    ...
    // Drop the in-memory CEK so subsequent encrypt/decrypt calls throw.
    // The frontend handles localStorage CEK cleanup separately.
    this.encryption.forget();
    console.debug('[Storage] Encryption key forgotten');

    // Mark uninitialized so a future `init` reseeds defaults via the
    // normal lifecycle.
  ...
  /**
   * Initialize storage. The adapter is decided at construction time
   * (`createStorage` or `GremlinServer.init()` builds the right one from
   * the storage config), so this just brings up the adapter and seeds
   * defaults. The caller MUST have already loaded a CEK into the
   * encryption core via `initializeWithCEK` — this method throws if not.
   */
  async initialize(): Promise<void> {
    ...
    // Encryption is the caller's responsibility. We type the field as
    // `EncryptionCore`, which has no `initialize()` lifecycle of its own —
    // the only way to load a key is `initializeWithCEK`, which the worker
    // calls inside `GremlinServer.init` before constructing this storage
    // instance.
    if (!this.encryption.isInitialized()) {
      throw new Error(
        'UnifiedStorage.initialize: encryption core has no CEK — call initializeWithCEK first'
      );
    }
  ...
  async getModelsWithCache(apiDefinitionId: string): Promise<{
    ...
    try {
      ...
      const json = await this.encryption.decryptWithDecompression(record.encryptedData);
  ...
  async saveModels(apiDefinitionId: string, models: Model[]): Promise<void> {
    ...
    const encrypted = await this.encryption.encryptWithCompression(JSON.stringify(data), true);
  ...
  async getMessages(chatId: string): Promise<Message<unknown>[]> {
    ...
    for (const record of records) {
      try {
        // Automatically detects and decompresses if "GZ" prefix present
        const decrypted = await this.encryption.decryptWithDecompression(record.encryptedData);
      ...
      for (const message of messages) {
        try {
          ...
          try {
            ...
            const encryptionSvc = this.encryption as unknown as {
          ...
          if (isCompressed) {
            ...
            const encrypted = await this.encryption.encryptWithCompression(
              JSON.stringify(data),
              true
            );
            await this.adapter.save(Tables.MESSAGES, message.id, encrypted, {
