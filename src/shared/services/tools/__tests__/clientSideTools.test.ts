...
import { ClientSideToolRegistry, executeToolSimple } from '../clientSideTools';
import {
  type ClientSideTool,
  type ToolResult,
...
// clientSideTools tests don't reach into vfs adapter methods.
const mockAdapter = {} as VfsAdapter;
const mockAdapterFactory = (_ns?: string) => mockAdapter;

// Backend deps that route through the locally-constructed registry so
// `executeClientSideTool` (which now reads from `context.toolRegistry`)
// finds the tools registered by the test.
const localBackendDeps = { ...stubBackendDeps, toolRegistry };
