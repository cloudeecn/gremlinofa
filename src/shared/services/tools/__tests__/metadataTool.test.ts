...
import type { Chat, ToolContext, ToolResult } from '../../../protocol/types';
...
// metadataTool's tests don't exercise vfs adapter methods — the empty
// stub satisfies ToolContext's shape.
const mockAdapter = {} as VfsAdapter;
const mockAdapterFactory = (_ns?: string) => mockAdapter;

// Per-test stub `storage` object — passed through `ToolContext.storage`
// instead of mocking the singleton module. metadataTool now reads
// `context.storage`, so the stub here is what the tool actually exercises.
const mockStorage = {
  getChats: vi.fn(),
  getProject: vi.fn(),
...
describe('metadataTool', () => {
  ...
  describe('list_recent_chats', () => {
    function setMockStorage(
      ...
      (mockStorage.getChats as ReturnType<typeof vi.fn>).mockResolvedValue(chats);
      (mockStorage.getProject as ReturnType<typeof vi.fn>).mockResolvedValue(
