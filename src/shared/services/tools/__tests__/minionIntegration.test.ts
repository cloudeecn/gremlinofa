...
// to be re-imported via the source modules (which no longer export the
// singletons after Phase 4 follow-up).
const mocks = vi.hoisted(() => ({
  apiService: {
    sendMessageStream: vi.fn(),
    extractToolUseBlocks: vi.fn(),
    mapStopReason: vi.fn((_: unknown, reason: string | null) => reason || 'end_turn'),
...
// Mock dependencies. The four hoisted mock holders below are referenced
// from the source modules' vi.mock factories so that any code path that
// dynamically `await import('../../storage')` (etc.) sees the same mocked
// surfaces this test configures via `mocks.X.mockResolvedValue(...)`.
vi.mock('../../api/apiService', () => ({
  APIService: class MockAPIService {
    sendMessageStream = mocks.apiService.sendMessageStream;
    extractToolUseBlocks = mocks.apiService.extractToolUseBlocks;
    mapStopReason = mocks.apiService.mapStopReason;
    shouldPrependPrefill = mocks.apiService.shouldPrependPrefill;
  },
...
vi.mock('../../storage', () => ({
  ...
  createStorage: () => mocks.storage,
  createStorageAdapter: () => ({}),
}));

vi.mock('../../../engine/lib/api/modelMetadata', () => ({
...
// Build the storage mock methods. They live on the hoisted `mocks.storage`
// holder so this test references them via the same name as before. The
// implementations below close over `mockStorageData` defined immediately
// above this block, which works because vi.hoisted only hoisted the
// holder declaration; assigning method implementations happens after
// `mockStorageData` is in scope.
mocks.storage.getProject = vi.fn((id: string) =>
  Promise.resolve(mockStorageData.projects.get(id))
) as ReturnType<typeof vi.fn>;
mocks.storage.getAPIDefinition = vi.fn((id: string) =>
  Promise.resolve(mockStorageData.apiDefinitions.get(id))
) as ReturnType<typeof vi.fn>;
mocks.storage.getModel = vi.fn((apiDefId: string, modelId: string) =>
  Promise.resolve(mockStorageData.models.get(`${apiDefId}:${modelId}`))
) as ReturnType<typeof vi.fn>;
mocks.storage.getMinionChat = vi.fn((id: string) =>
  Promise.resolve(mockStorageData.minionChats.get(id))
) as ReturnType<typeof vi.fn>;
mocks.storage.getMinionMessages = vi.fn((chatId: string) =>
  Promise.resolve(mockStorageData.minionMessages.get(chatId) ?? [])
) as ReturnType<typeof vi.fn>;
mocks.storage.saveMinionChat = vi.fn((chat: MinionChat) => {
  mockStorageData.minionChats.set(chat.id, chat);
  return Promise.resolve();
}) as ReturnType<typeof vi.fn>;
mocks.storage.saveMinionMessage = vi.fn((chatId: string, msg: Message<unknown>) => {
  const messages = mockStorageData.minionMessages.get(chatId) ?? [];
  messages.push(msg);
  mockStorageData.minionMessages.set(chatId, messages);
  return Promise.resolve();
}) as ReturnType<typeof vi.fn>;
mocks.storage.getAttachments = vi.fn(() => Promise.resolve([])) as ReturnType<typeof vi.fn>;
mocks.storage.deleteMessagesAfter = vi.fn((chatId: string, messageId: string) => {
  const messages = mockStorageData.minionMessages.get(chatId) ?? [];
...
mocks.storage.deleteMessageAndAfter = vi.fn((chatId: string, messageId: string) => {
  const messages = mockStorageData.minionMessages.get(chatId) ?? [];
  const idx = messages.findIndex(m => m.id === messageId);
  if (idx >= 0) {
...
vi.mock('../clientSideTools', () => ({
  ...
  ClientSideToolRegistry: class MockToolRegistry {
    get = mocks.toolRegistry.get;
    getSystemPrompts = mocks.toolRegistry.getSystemPrompts;
  },
...
const minionAdapter = {} as VfsAdapter;
const minionAdapterFactory = (_ns?: string) => minionAdapter;

// Surface the hoisted mocks under the same names the body of this test
// file uses (`apiService.X`, `storage.X`, `toolRegistry.X`). Each of these
// references the singleton-replacement objects from the `mocks` holder.
const apiService = mocks.apiService;
const storage = mocks.storage;
const toolRegistry = mocks.toolRegistry;

// Backend dependency bundle threaded through `ToolContext`. The mocked
// instances are re-used here so test setup that calls `vi.mocked(apiService.X)`
// and `mockStorageData.set(...)` keeps working — minionTool reads its
// collaborators via `context.*` instead of importing them directly.
const mockMinionDeps = {
  ...stubBackendDeps,
  storage: storage as unknown as (typeof stubBackendDeps)['storage'],
  apiService: apiService as unknown as (typeof stubBackendDeps)['apiService'],
  toolRegistry: toolRegistry as unknown as (typeof stubBackendDeps)['toolRegistry'],
};
...
describe('Minion Integration', () => {
  ...
  describe('injectFiles error handling', () => {
    // After Phase 2, `minionTool` reads injected files via
    // `context.createVfsAdapter().readFile(...)` instead of the
    // module-level `vfsService.readFile`. Each test below builds an
    // adapter factory whose `readFile` rejects with the expected error.
    function buildFailingAdapterFactory(failPath?: string) {
      const adapter = {
        readFile: vi.fn(async (path: string) => {
          throw new Error(failPath ? `Path not found: ${failPath}` : `Path not found: ${path}`);
        }),
    ...
    it('returns error to caller when injected file not found', async () => {
      const failingFactory = buildFailingAdapterFactory('/nonexistent.ts');
  ...
  // ============================================================================
  // LoopRegistry integration — verifies that minion sub-loops register
  // themselves with the per-server LoopRegistry so the sidebar Running Loops
  // UI can show them as indented children of the parent loop and abort one
  // independently of its siblings.
  // ============================================================================
  describe('LoopRegistry child-loop registration', () => {
    it('registers a child loop on the registry with the parent loopId', async () => {
      // Use a real LoopRegistry so we can observe register / end calls.
      const { LoopRegistry } = await import('../../../engine/LoopRegistry');
      const realRegistry = new LoopRegistry();
      ...
      const context: ToolContext = {
        ...
        createVfsAdapter: minionAdapterFactory,
        signal: new AbortController().signal,
        // Critical: provide a parent loopId so registration kicks in
        // (the implementation skips registration when undefined to keep
        // standalone/test paths from polluting the registry).
        loopId: 'parent_loop_under_test',
        ...mockMinionDeps,
        loopRegistry: realRegistry as unknown as (typeof stubBackendDeps)['loopRegistry'],
      };

      // Track register/end calls so we can assert exact pairing.
      const registerSpy = vi.spyOn(realRegistry, 'register');
      const endSpy = vi.spyOn(realRegistry, 'end');

      await collectToolResult(
        minionTool.execute({ message: 'parallel work', displayName: 'Coder' }, toolOptions, context)
      );

      expect(registerSpy).toHaveBeenCalledTimes(1);
      const [activeLoop] = registerSpy.mock.calls[0];
      expect(activeLoop).toMatchObject({
        chatId: 'parent_chat', // groups under the user's chat, not the minion chat
        parentLoopId: 'parent_loop_under_test',
        status: 'running',
        apiDefinitionId: 'api_test',
        modelId: 'claude-3-sonnet',
        displayName: 'Coder',
      });
      expect(activeLoop.loopId).toMatch(/^loop_/);

      // Cleanup must run regardless of return path. The minion completed
      // normally so the registry sees a `complete` terminal status.
      expect(endSpy).toHaveBeenCalledTimes(1);
      expect(endSpy.mock.calls[0]).toEqual([activeLoop.loopId, 'complete']);
      // Registry is empty after the minion finishes.
      expect(realRegistry.list()).toEqual([]);
    });

    it('skips registration when context.loopId is undefined (standalone path)', async () => {
      // Standalone / test invocations have no parent loop. The minion
      // tool should run normally without polluting the registry — that's
      // the escape hatch for unit tests that don't care about the
      // sidebar plumbing.
      const { LoopRegistry } = await import('../../../engine/LoopRegistry');
      const realRegistry = new LoopRegistry();
    ...
    it('parent abort cascades into the child controller via the wired listener', async () => {
      // Verifies the parent → child cascade: aborting the parent's signal
      // (which the minion tool listens to via addEventListener) fires the
      // child's controller. The reverse direction is asserted indirectly
      // by the implementation comments — the child controller is local
      // to executeMinion and never reaches back into the parent.
      const { LoopRegistry } = await import('../../../engine/LoopRegistry');
      const realRegistry = new LoopRegistry();

      // Capture the signal the agentic loop actually saw so we can
      // assert it's wired to the child controller, not the parent.
      let observedSignal: AbortSignal | null = null;
      vi.mocked(apiService.sendMessageStream).mockImplementation(((options: {
        signal: AbortSignal;
      }) => {
      ...
      // The signal the child loop received MUST NOT be the parent signal —
      // that's the whole point of giving the child its own controller.
      expect(observedSignal).not.toBeNull();
      expect(observedSignal).not.toBe(parentController.signal);
    });

    it('end() runs even when the minion errors out (finally cleanup)', async () => {
      const { LoopRegistry } = await import('../../../engine/LoopRegistry');
      const realRegistry = new LoopRegistry();

      // Make the loop throw by failing to set up the API mock — the minion
      // tool's catch block converts it to an error tool result, but the
      // finally block must still call registry.end().
      vi.mocked(apiService.sendMessageStream).mockImplementation(() => {
      ...
      // Tool surfaces the error in its result content.
      expect(result.isError).toBe(true);
      // Registry was still cleaned up — terminal status defaults to 'error'.
      expect(endSpy).toHaveBeenCalledTimes(1);
      expect(endSpy.mock.calls[0][1]).toBe('error');
      expect(realRegistry.list()).toEqual([]);
    });
  });
});
