 ...
 *
 * Uses `context.createVfsAdapter` to read the project's memory tree.
 * VFS access lives only in the worker; `buildLoopOptions` always
...
async function getMemorySystemPrompt(
  ...
  const { apiType, namespace } = context;
  ...
  if (!context.createVfsAdapter) {
    throw new Error(
      'memoryTool.getMemorySystemPrompt: SystemPromptContext.createVfsAdapter is required. ' +
        'VFS access is only available inside the worker — direct service-layer calls are unsupported.'
    );
  }
  const adapter: VfsAdapter = context.createVfsAdapter(namespace);
