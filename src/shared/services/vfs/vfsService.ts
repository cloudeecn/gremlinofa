...
import type { VfsTree, VfsNode, VfsFile, VfsVersion } from '../../protocol/types';
...
import {
  ...
  getPathSegments,
  isRootPath,
} from '../../engine/lib/vfsPaths';

// ============================================================================
// Path Utilities
// ============================================================================
//
// The pure path helpers live in `src/lib/vfsPaths.ts` so frontend code can
// import them without crossing the services boundary. We re-export them
// here so existing internal callers (and the `src/services/vfs/index.ts`
// barrel) keep working unchanged.
export { normalizePath, getParentDir, getBasename, getPathSegments, isRootPath };
...
// ============================================================================
// Stat / Version / Orphan Result Types
// ============================================================================
...
// ============================================================================
// Pure helpers (no storage / encryption deps)
// ============================================================================

const _MS_HOUR_FOR_HELPERS = 3_600_000;
const _MS_DAY_FOR_HELPERS = 86_400_000;
const _MS_WEEK_FOR_HELPERS = 7 * _MS_DAY_FOR_HELPERS;
const _MS_YEAR_FOR_HELPERS = 365 * _MS_DAY_FOR_HELPERS;

interface _VersionEntryForHelpers {
  version: number;
  createdAt: number;
}
 ...
 * Pure — no storage / encryption deps.
 */
 ...
 * - > 1yr: discard
 *
 * For bucketed tiers, the highest version number in each bucket wins. Pure
 * function — no storage / encryption deps.
 */
export function selectVersionsToKeep(
  versions: _VersionEntryForHelpers[],
  now: number
): Set<number> {
  const keep = new Set<number>();

  const hourlyBuckets = new Map<number, _VersionEntryForHelpers>();
  const dailyBuckets = new Map<number, _VersionEntryForHelpers>();
  const weeklyBuckets = new Map<number, _VersionEntryForHelpers>();

  for (const entry of versions) {
    const age = now - entry.createdAt;

    if (age <= _MS_DAY_FOR_HELPERS) {
      keep.add(entry.version);
    } else if (age <= 3 * _MS_DAY_FOR_HELPERS) {
      const bucket = Math.floor(entry.createdAt / _MS_HOUR_FOR_HELPERS);
      const existing = hourlyBuckets.get(bucket);
      if (!existing || entry.version > existing.version) {
        hourlyBuckets.set(bucket, entry);
      }
    } else if (age <= 30 * _MS_DAY_FOR_HELPERS) {
      const bucket = Math.floor(entry.createdAt / _MS_DAY_FOR_HELPERS);
      const existing = dailyBuckets.get(bucket);
      if (!existing || entry.version > existing.version) {
        dailyBuckets.set(bucket, entry);
      }
    } else if (age <= _MS_YEAR_FOR_HELPERS) {
      const bucket = Math.floor(entry.createdAt / _MS_WEEK_FOR_HELPERS);
...
/**
 * Create a VFS service bound to a `UnifiedStorage` and an `EncryptionCore`.
 *
 * Phase 3 of the singleton-encapsulation refactor: instead of importing
 * `storage` and `encryptionService` from the module-level singletons,
 * `GremlinServer.init()` calls this factory with the per-server instances
 * it built from `InitParams`. The returned service object closes over
 * those instances, so every internal helper (`loadTree`, `saveFile`, ...)
 * and every public method (`createFile`, `readFile`, ...) routes through
 * the injected pair instead of the singletons.
 *
 * Phase 1.5 narrowed the encryption parameter to `EncryptionCore` so the
 * VFS service can never reach for browser-only methods. After Phase 1.65,
 * `EncryptionCore` is the *only* encryption flavor — the legacy
 * main-thread subclass was deleted, so the parameter type is now both
 * the design intent and the only thing that exists.
 */
export function createVfsService(storage: UnifiedStorage, encryptionService: EncryptionCore) {
  // ==========================================================================
  // Storage Helpers
  // ==========================================================================
   ...
   */
  async function readFile(projectId: string, path: string, namespace?: string): Promise<string> {
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
    const { node } = navigateToNode(tree, normalized);
   ...
   */
  async function deleteFile(projectId: string, path: string, namespace?: string): Promise<void> {
    assertWritable(path, namespace);
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
   ...
   */
  async function mkdir(projectId: string, path: string, namespace?: string): Promise<void> {
    assertWritable(path, namespace);
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
    const basename = getBasename(normalized);
   ...
   */
  async function exists(projectId: string, path: string, namespace?: string): Promise<boolean> {
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
   ...
   */
  async function isFile(projectId: string, path: string, namespace?: string): Promise<boolean> {
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
    const { node } = navigateToNode(tree, normalized);
   ...
   */
  async function restore(projectId: string, path: string, namespace?: string): Promise<void> {
    assertWritable(path, namespace);
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
    const { node } = navigateToNode(tree, normalized);
   ...
   */
  async function purge(projectId: string, path: string, namespace?: string): Promise<void> {
    assertWritable(path, namespace);
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
   ...
   */
  async function hasVfs(projectId: string): Promise<boolean> {
    const adapter = storage.getAdapter();
    const metaId = `vfs_meta_${projectId}`;
    const record = await adapter.get(Tables.VFS_META, metaId);
    return record !== null;
   ...
   */
  async function stat(projectId: string, path: string, namespace?: string): Promise<VfsStat> {
    const tree = await loadTree(projectId);
    const normalized = resolveNamespacedPath(path, namespace);
   ...
   */
  async function getFileId(projectId: string, path: string): Promise<string | null> {
    const tree = await loadTree(projectId);
    const normalized = normalizePath(path);
    const { node } = navigateToNode(tree, normalized);
   ...
   */
  async function listVersions(_projectId: string, fileId: string): Promise<VersionInfo[]> {
    const currentFile = await loadFile(fileId);
   ...
   */
  async function listOrphans(projectId: string): Promise<OrphanInfo[]> {
    const tree = await loadTree(projectId);
   ...
   */
  async function purgeOrphan(projectId: string, fileId: string): Promise<void> {
    const tree = await loadTree(projectId);
  ...
  // ============================================================================
  //
  // `selectVersionsToKeep` is hoisted to module scope (above the factory)
  // so frontend tests can call it without instantiating a service. The
  // local `MS_*` constants below are unused now that the helper is
  // hoisted; the week-old cutoff for `compactProject` re-derives them
  // inline.
  ...
  async function pruneFileVersions(
    ...
    const entries = historicalVersions.map(v => ({
      version: v.version,
