...
export class RemoteVfsAdapter implements VfsAdapter {
  ...
  private encryptionService: EncryptionCore;

  /**
   * @param encryptionService Per-server encryption core. Phase 3 of the
   * singleton refactor: the adapter no longer imports the module-level
   * `encryptionService` singleton — `vfsFacade.getAdapter` passes the
   * instance through from `BackendDeps`. Only used when `encrypt=true`.
   */
  constructor(
    baseUrl: string,
    userId: string,
    ...
    projectId: string,
    encrypt: boolean,
    encryptionService: EncryptionCore,
    namespace?: string
  ) {
